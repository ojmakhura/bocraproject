module.controller(
  ...
  function (
    ...
    $scope.addRealmRole = function () {
      $scope.realmRolesToAdd = JSON.parse(
        "[" + $scope.selectedRealmRoles + "]"
      );
      $scope.selectedRealmRoles = [];
      $http
    ...
    $scope.addClientRole = function () {
      $scope.clientRolesToAdd = JSON.parse(
        "[" + $scope.selectedClientRoles + "]"
      );
      $http
        .post(
        ...
        function () {
          UserFederatedIdentity.remove(
            ...
            function () {
              ...
              var indexToRemove = $scope.federatedIdentities.indexOf(
                providerLink
              );
...
module.controller(
  ...
  function ($scope, $location, realm, user, client, offlineSessions) {
    $scope.realm = realm;
    $scope.user = user;
    $scope.client = client;
...
module.controller(
  "UserTabCtrl",
  function ($scope, $location, Dialog, Notifications, Current) {
    ...
    RequiredActions.query({ realm: realm.realm }, function (data) {
      ...
      console.log(
        "ng-model: user.requiredActions=" +
          JSON.stringify($scope.user.requiredActions)
      );
      console.log("---------------------");
      console.log(
        "ng-repeat: userReqActionList=" +
          JSON.stringify($scope.userReqActionList)
      );
      console.log("---------------------");
    });
    $scope.$watch(
...
module.controller(
  ...
  function (
    ...
    $scope.getUserStorageProviderName = function () {
      return user.federationLink
        ? $scope.federationLinkName
        : $scope.originName;
    };
    ...
    function loadCredentials() {
      ...
      UserCredentials.getConfiguredUserStorageCredentialTypes(
        { realm: realm.realm, userId: user.id },
        null,
        function (userStorageCredentialTypes) {
...
module.controller(
  "UserCredentialsDataModalCtrl",
  function ($scope, credentialData) {
  ...
  function (
    ...
    $scope.save = function () {
      ...
      if (
        ...
        !parseInt($scope.instance.config["batchSizeForSync"][0])
      ) {
    ...
    var initConnectionTest = function (testAction, ldapConfig) {
      return {
        ...
        bindCredential:
          ldapConfig.bindCredential && ldapConfig.bindCredential[0],
        useTruststoreSpi:
          ldapConfig.useTruststoreSpi && ldapConfig.useTruststoreSpi[0],
        connectionTimeout:
          ldapConfig.connectionTimeout && ldapConfig.connectionTimeout[0],
        ...
        .then(
          function (response) {
            ...
            if (angular.isArray(ldapOids)) {
              const passwordModifyOid = ldapOids.filter(function (ldapOid) {
                return ldapOid.oid === PASSWORD_MODIFY_OID;
              });
    ...
    $scope.removeUserFederation = function () {
      Dialog.confirmDelete($scope.instance.name, "ldap provider", function () {
        $scope.instance.$remove(
          ...
          function () {
            $location.url(
              "/realms/" + Current.realm.realm + "/user-federation"
            );
