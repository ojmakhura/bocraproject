...
module.controller(
  "RealmTabCtrl",
  function (Dialog, $scope, Current, Realm, Notifications, $location) {
...
module.controller(
  "RealmDropdownCtrl",
  function ($scope, Realm, Current, Auth, $location) {
...
module.controller(
  ...
  function (
    ...
    if (
      !$scope.realm.hasOwnProperty("defaultRoles") ||
      $scope.realm.defaultRoles === null
    ) {
    ...
    $scope.deleteRealmDefaultRole = function () {
      ...
      for (var i = 0; i < $scope.selectedRealmDefRoles.length; i++) {
        ...
        var index = $scope.realm.defaultRoles.indexOf(
          $scope.selectedRealmDefRoles[i]
        );
        if (index > -1) {
    ...
    $scope.changeClient = function (client) {
      ...
      if ($scope.selectedClient) {
        ClientRole.query(
          { realm: $scope.realm.realm, client: $scope.selectedClient.id },
          function (appDefaultRoles) {
            if (
              !$scope.selectedClient.hasOwnProperty("defaultRoles") ||
              $scope.selectedClient.defaultRoles === null
            ) {
...
module.controller(
  ...
  function (Dialog, $scope, Current, Notifications, $location) {
    $scope.removeIdentityProvider = function () {
      Dialog.confirmDelete(
        $scope.identityProvider.alias,
        "provider",
        function () {
...
module.controller(
  ...
  function (
    ...
    $scope.addProvider = function (provider) {
      $location.url(
        "/create/identity-provider/" + realm.realm + "/" + provider.id
      );
    };
        ...
        IdentityProvider.save(
          ...
          function () {
            $location.url(
              "/realms/" +
                realm.realm +
                "/identity-provider-settings/provider/" +
                $scope.identityProvider.providerId +
                "/" +
                $scope.identityProvider.alias
            );
            Notifications.success(
    ...
    $scope.$watch(
      ...
      function () {
        ...
        if (
          ...
          $scope.actionTokenAttribute.hasOwnProperty("time")
        ) {
...
module.controller(
  ...
  function (
    ...
    $scope.addProvider = function (provider) {
      $location.url(
        "/create/keys/" + realm.realm + "/providers/" + provider.id
      );
    };
    ...
    $scope.logoutAll = function () {
      RealmLogoutAll.save({ realm: realm.realm }, function (globalReqResult) {
        ...
        if (failedCount > 0) {
          var msgStart =
            successCount > 0
              ? "Successfully logout all users under: " +
                globalReqResult.successRequests +
                " . "
              : "";
          Notifications.error(
            msgStart +
              "Failed to logout users under: " +
              globalReqResult.failedRequests +
              ". Verify availability of failed hosts and try again"
          );
        } else {
...
module.controller(
  ...
  function (
    ...
    $scope.setNotBeforeNow = function () {
      Realm.update(
        { realm: realm.realm, notBefore: new Date().getTime() / 1000 },
        function () {
    ...
    $scope.pushRevocation = function () {
      RealmPushRevocation.save(
        { realm: realm.realm },
        function (globalReqResult) {
          var successCount = globalReqResult.successRequests
            ? globalReqResult.successRequests.length
            : 0;
          var failedCount = globalReqResult.failedRequests
          ...
          if (successCount == 0 && failedCount == 0) {
            ...
            var msgStart =
              successCount > 0
                ? "Successfully push notBefore to: " +
                  globalReqResult.successRequests +
                  " . "
                : "";
            Notifications.error(
              msgStart +
                "Failed to push notBefore to: " +
                globalReqResult.failedRequests +
                ". Verify availability of failed hosts and try again"
            );
          } else {
...
module.controller(
  "RealmEventsCtrl",
  function ($scope, RealmEvents, realm, serverInfo) {
    $scope.realm = realm;
    $scope.page = 0;
...
module.controller(
  ...
  function ($scope, RealmAdminEvents, realm, serverInfo, $modal, $filter) {
    $scope.realm = realm;
    $scope.page = 0;
    ...
    $scope.viewRepresentation = function (event) {
      $modal.open({
        templateUrl:
          resourceUrl +
          "/partials/modal/realm-events-admin-representation.html",
    ...
    $scope.viewAuth = function (event) {
      $modal.open({
        templateUrl:
          resourceUrl + "/partials/modal/realm-events-admin-auth.html",
        controller: "RealmAdminEventsModalCtrl",
        resolve: {
          event: function () {
            return event;
...
module.controller(
  "RealmAdminEventsModalCtrl",
  function ($scope, $filter, event) {
    $scope.event = event;
  }
);
...
module.controller(
  ...
  function ($scope, realm, identityProvider, mapperTypes, mappers) {
    $scope.realm = realm;
...
module.controller(
  "CreateFlowCtrl",
  function ($scope, realm, AuthenticationFlows, Notifications, $location) {
  ...
  function (
    ...
    $scope.register = function () {
      ...
      $modal.open({
        templateUrl:
          resourceUrl +
          "/partials/modal/unregistered-required-action-selector.html",
    ...
    $scope.done = function () {
      ...
      Dialog.open(title, message, btns, function () {
        $location.url(
          "/realms/" +
            realm.realm +
            "/client-registration/client-initial-access"
        );
      });
    };
  }
  ...
  function (
    ...
    $scope.addProvider = function (authType, provider) {
      ...
      $location.url(
        "/realms/" +
          realm.realm +
          "/client-registration/client-reg-policies/create/" +
          authType +
          "/" +
          provider.id
      );
    ...
    $scope.getInstanceLink = function (instance) {
      return (
        "/realms/" +
        realm.realm +
        "/client-registration/client-reg-policies/" +
        instance.providerId +
        "/" +
        instance.id
      );
    ...
    if ($scope.create) {
      ...
      if ($scope.providerType.properties) {
        for (let i = 0; i < $scope.providerType.properties.length; i++) {
          ...
          $scope.instance.config[configProperty.name] = toDefaultValue(
            configProperty
          );
        }
      }
...
module.controller(
  "RealmImportCtrl",
  function ($scope, realm, $route, Notifications, $modal, $resource) {
    $scope.rawContent = {};
    $scope.fileContent = {
      enabled: true,
    };
...
module.controller(
  "RealmExportCtrl",
  function ($scope, realm, $http, $httpParamSerializer, Notifications, Dialog) {
    $scope.realm = realm;
    ...
    function download() {
      var exportUrl =
        authUrl + "/admin/realms/" + realm.realm + "/partial-export";
      var params = {};
