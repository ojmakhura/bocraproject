...
import { AfterViewInit, Component, ElementRef, Injector, Input, OnDestroy, OnInit, QueryList, ViewChild, ViewChildren } from '@angular/core';
...
export class ReportChartComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  @ViewChildren(BaseChartDirective) pieCharts: QueryList<BaseChartDirective>;
  @ViewChildren("pieChartDiv") pieChartDiv: QueryList<ElementRef>;
    ...
    this.labelNames =
      this.period === 'all'
        ? this.gridColumnHeaders
            ...
            .map((h) => h.label)
        : this.gridColumnHeaders.filter((g) => g.period === this.period).map((h) => h.label);
  ...
  selectedChartType() {
    if (this.chartType === 'bar') {
      ...
      this.createPieChartDatasets();
    }
  }

  // TODO: complete this
  createPieChartDatasets() {
    let pieData = {};
    let pieCharts = {};
    ...
    this.selectedPeriods.forEach((per) => {
      let pie = pieData[per.alias];

      if(!pieCharts[per.alias]) {
        pieCharts[per.alias] = [];
      }

      Object.values(pie).forEach((p: any) => {
        pieCharts[per.alias].push({
          type: 'pie',
          labels: p.labels,
          label: p.label,
          datasets: p.datasets,
    ...
    this.pieChartDatasets = pieCharts;
  ...
  getChartImageData(): any {
    if (this.chartType === 'table') {
      ...
      /**
       * We are working with the HTML elements here, not angular objets
       */
      ...
      this.pieChartDiv.forEach((div: ElementRef) => {
        let label: string = div.nativeElement.querySelector("h3").innerText;
        let im = imageData['images'];
        im[label] = [];
        
        let chartDivs = Object.values(div.nativeElement.children).slice(1, div.nativeElement.children.length)

        chartDivs.forEach((chartDiv: any) => {
          
          let dataLabel = chartDiv.children[0].innerText
          let canvas = chartDiv.children[1];
          
          im[label].push( {
            dataLabel: dataLabel,
            image: canvas.toDataURL(), // got the html element, not the Chart object
            pieChartCaption: chartDiv.children[2].value
          });
        });
