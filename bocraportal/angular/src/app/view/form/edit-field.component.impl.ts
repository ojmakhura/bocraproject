...
import { EditFieldAddExpressionForm } from '@app/view/form/edit-field.component';
  ...
  override afterEditFieldDelete(form: EditFieldDeleteForm): void {
    if(form?.field?.id) {
      if(confirm("Are you sure to delete the expression configuration?")) {
        this.store.dispatch(FieldActions.remove({id: form.field.id, loading: true}));
        this.editFieldFormReset();
      }
    }
  }
