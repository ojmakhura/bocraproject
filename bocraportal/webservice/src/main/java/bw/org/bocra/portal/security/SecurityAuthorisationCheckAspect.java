...
public class SecurityAuthorisationCheckAspect {
    ...
    public Boolean isAuthorised(JoinPoint joinPoint) throws Throwable {
        ...
        Method method = joinPoint.getSignature().getDeclaringType().getMethod(joinPoint.getSignature().getName(), Long.class);
        ...
        boolean authorised = StringUtils.isBlank(methodCall) ? this.isAuthorised(path) : this.handleIsAuthorised(methodCall + " " + path);
